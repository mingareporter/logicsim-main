...
paper.on({
    ...
    'blank:pointerdown': function (evt) {
        ...
        paperContent.css("cursor", "grabbing");
        $("body").on("mousemove", function (evt) {
            var changes = {
...
app.getParseMode = function () {
    var sel = document.getElementById("parseMode");
    return (sel && sel.value === "rpn") ? "rpn" : "infix";
};
...
app.renderTruthTable = function (table) {
    ...
    for (var i = 0; i < table.vars.length; i++) {
        html += '<th>' + app.escapeHtml(table.vars[i]) + '</th>';
    }
    html += '<th class="tt-result">结果</th></tr></thead><tbody>';
    for (var r = 0; r < table.rows.length; r++) {
        var row = table.rows[r];
        html += '<tr class="' + (row.result ? "tt-true" : "") + '">';
        for (var c = 0; c < row.vals.length; c++) {
            html += '<td>' + row.vals[c] + '</td>';
        }
        html += '<td class="tt-result">' + row.result + '</td></tr>';
    }
    html += '</tbody></table>';
    var meta = '<div class="tt-meta">' + table.vars.length + ' 个变量 · ' + table.rows.length + ' 行</div>';
    container.innerHTML = meta + html;
};

app.updateStatusOk = function (mode) {
    status.textContent = (mode === "rpn" ? "逆波兰表达式" : "中缀表达式") + "解析完成，真值表已生成。";
    status.style.color = "";
};

app.updateStatusError = function (e) {
    var msg = (e && e.message) ? e.message : String(e);
    var pos = (e && typeof e.position === "number") ? "（位置 " + (e.position + 1) + "）" : "";
    status.textContent = "解析失败：" + msg + pos;
...
app.toggleTheme = function () {
    ...
    try { localStorage.setItem("logicsim-theme", next); } catch (e) { }
    app.syncThemeButton();
};

app.syncThemeButton = function () {
...
app.initTheme = function () {
    var saved = null;
    try { saved = localStorage.getItem("logicsim-theme"); } catch (e) { }
    var theme = saved;
...
app.parseLogic = function () {
    ...
    var mode = app.getParseMode();
    var knownErrors = [
        "Empty String!",
        "use wrong Name as variable",
        "Format Error: arguments less than needed",
        "Format Error: arguments more than needed"
    ];
    var knownErrorMap = {
        "Empty String!": "表达式为空",
        "use wrong Name as variable": "变量名使用错误",
        "Format Error: arguments less than needed": "格式错误：操作数不足",
        "Format Error: arguments more than needed": "格式错误：操作数过多"
    };
    try {
        var analysis = Expression.analyze(exprText, mode);
        var resultParsed = LogicParser(analysis.rpnString);
        if ("string" == typeof (resultParsed)) {
            if (knownErrors.indexOf(resultParsed) >= 0) {
                throw { message: knownErrorMap[resultParsed], position: null };
            }
            // 单个变量/常量：LogicParser 返回其名字符串，属合法输入
        }
        var viewModel = ViewGen(ModelGen(resultParsed));
        origin = viewModel;
        document.getElementById("myModel").value = JSON.stringify(origin);
        app.renderTruthTable(analysis.truthTable);
        app.updateStatusOk(mode);
    } catch (e) {
        app.updateStatusError(e);
    }
};
...
/* 已移除无用的 updateGraph 死代码（原引用了不存在的 chosedElement.Sheets/Fields） */
...
app.UpdateOption = function () {
    ...
    var items = Array.from(ImportNames).map(function (name) {
        return '<span class="chip">' + app.escapeHtml(name) + '</span>';
    });
    varList.innerHTML = items.length ? items.join("") : '<span class="muted">暂无变量</span>';
}
